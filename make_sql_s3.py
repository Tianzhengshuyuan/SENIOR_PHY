...
def extract_catalog_and_chapters_with_pages(pdf_path, output_sql_path):
    ...
    for line in remaining_lines:
        ...
        if line.startswith("第") and "章" in line:
            if re.match(r"第[一二三四五六七八九十百千]", line):
                # 如果当前章节未结束，保存到章节列表
                if current_chapter:
    ...
    with open(output_sql_path, "w", encoding="utf-8") as sql_file:
        ...
        with pdfplumber.open(pdf_path) as pdf:
            for chapter_index, chapter in enumerate(chapters):
                # 插入章信息到 SQL文件
                chapter_id = f"010103{str(chapter_index+1).zfill(2)}000000"
                ...
                sql_file.write(
                    ...
                    f"VALUES \n"
                    f"('{chapter_id}', 'PHYSICS', 'SENIOR', 'TERM_3', "
                    f"'{chapter_code}', '{chapter['title']}', '', NULL),\n"
                )
                
                for section_index, section in enumerate(chapter["sections"]):
                    # 插入节信息到 SQL文件
                    section_id = f"010103{str(chapter_index+1).zfill(2)}{str(section_index+1).zfill(2)}0000"
                    section_code = f"{chapter_code}.SEC{str(section_index+1).zfill(2)}"
                    sql_file.write(
                        f"('{section_id}', 'PHYSICS', 'SENIOR', 'TERM_3', "
                    ...
                    for page_number in range(start_page, end_page + 1):
                        ...
                        while char_index < len(current_page.chars):
                            ...
                            # 检查字体和大小是否符合知识点标题的条件
                            if char.get("fontname") == "YOQJQH+FZLTZHK--GBK1-0" and round(char.get("size")) == 14 and char.get("text") != "问" and char.get("text") != "题":
                                ...
                                for next_char_index in range(char_index + 1, len(current_page.chars)):
                                    ...
                                    if next_char.get("fontname") == "YOQJQH+FZLTZHK--GBK1-0" and round(next_char.get("size")) == 14:
                                ...
                                # 插入知识点信息到 SQL文件
                                kp_id = f"010103{str(chapter_index+1).zfill(2)}{str(section_index+1).zfill(2)}{str(kp_index+1).zfill(2)}00"
                                kp_code = f"{section_code}.KP{str(kp_index+1).zfill(2)}"
                                sql_file.write(
                                    f"('{kp_id}', 'PHYSICS', 'SENIOR', 'TERM_3', "
                                ...
                                previous_font = "YOQJQH+FZLTZHK--GBK1-0"
                                previous_size = 14
                            # 检查知识点中的正文，是否符合条件
                                ...
                                # 如果遇到句子结束符，结束当前句子
                                if text in "。！？":
                                    proportion = calculate_font_proportion(current_sentence, "TLNPYR+FZSSK--GBK1-0", 12)
                                    ...
                                    if proportion >= 0.8:  # 字体比例需超过 80%
                                        ...
                                        if check_sentence(sentence_text):
                                            ...
                                            # 插入选项信息到 SQL文件
                                            op_id = f"010103{str(chapter_index+1).zfill(2)}{str(section_index+1).zfill(2)}{str(kp_index+1).zfill(2)}{str(op_index+1).zfill(2)}"
                                            op_code = f"{kp_code}.OP{str(op_index+1).zfill(2)}"
                                            sql_file.write(
                                                f"('{op_id}', 'PHYSICS', 'SENIOR', 'TERM_3', "
...
# 示例用法
pdf_path = "senior_physics_textbooks/senior_3.pdf"  # 替换为你的 PDF 文件路径
output_sql_path = "sql/s3.sql"
